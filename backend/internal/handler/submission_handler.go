...
// TestRun 试运行：跑题目前两条示例输入，不判对错、不留提交记录。
func (h *SubmissionHandler) TestRun(c *gin.Context) {
	userID := getUserID(c)
	problemID, err := primitive.ObjectIDFromHex(c.Param("id"))
	if err != nil {
		_ = c.Error(util.NewAppError(constants.CodeBadRequest, "无效的题目 ID"))
	...
	resp, err := h.submissionService.TestRun(c.Request.Context(), userID, problemID, &req)
	if err != nil {
		_ = c.Error(err)
		return
	}
